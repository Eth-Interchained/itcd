...
//!
//! Exposes NEDB's causal DAG storage to the ITC C++ node,
//! replacing LevelDB as the block index and chainstate backend.
//!
//! # Phase 1 (default — no features)
//! BTreeMap-backed in-process store.  Proves the C FFI surface, the
//! CDBWrapper shim, and the full CI pipeline.  Produces identical external
//! behaviour to LevelDB from the node's perspective.
//!
//! # Phase 2 (--features phase2)
//! `nedb_core_v2::Db` — content-addressed DAG with real BLAKE2b-512
//! chain head, MVCC `AS OF`, causal provenance (`caused_by`),
//! AES-256-GCM at-rest encryption (NEDB_TMK), and full TRACE.
//! Activate with: `cargo build --release --features phase2`
//!
//! The public C API surface is identical in both phases.
//!
...
// ─────────────────────────────────────────────────────────────────────────
// Handle types (cfg-branched — only one definition active at compile time)
// ─────────────────────────────────────────────────────────────────────────

/// Phase 1 internal store.
#[cfg(not(feature = "phase2"))]
struct NedbInner {
    store: BTreeMap<Vec<u8>, Vec<u8>>,
    seq:   u64,
    head:  Vec<u8>,   // running BLAKE2b-512 chain head
    name:  String,
}
...
/// Opaque database handle — Phase 1.
#[cfg(not(feature = "phase2"))]
pub struct NedbHandle {
    inner: Mutex<NedbInner>,
}

/// Opaque database handle — Phase 2.
#[cfg(feature = "phase2")]
pub struct NedbHandle {
    db:   Arc<Db>,
    coll: String,
...
// ─────────────────────────────────────────────────────────────────────────
// Iterator (same snapshot structure in both phases)
// ─────────────────────────────────────────────────────────────────────────

/// Snapshot iterator — entries captured at creation time, sorted by key.
pub struct NedbIter {
...
// ─────────────────────────────────────────────────────────────────────────
// Batch op (always present — used by nedb_batch_write)
// ─────────────────────────────────────────────────────────────────────────

/// A single operation in a batch write.  `value = NULL` means delete.
#[repr(C)]
pub struct NedbOp {
...
/// `dek`: AES-256-GCM key as a 64-char hex string, or NULL.
///        Phase 2 also reads NEDB_TMK from the environment.
#[no_mangle]
pub extern "C" fn nedb_open(path: *const c_char, _dek: *const c_char) -> *mut NedbHandle {
    ...
    #[cfg(feature = "phase2")]
    {
        ...
        // Collection name = last path component (e.g. "block_index", "chainstate")
        let coll = db_path
            .file_name()
            .map(|n| n.to_string_lossy().to_string())
            .unwrap_or_else(|| "itc".to_string());

        // DEK: read from NEDB_TMK env var first, then fall back to dek param.
...
/// Returns: 0 found (caller frees via nedb_free_value), 1 not found, -1 error.
#[no_mangle]
...
/// Phase 2: key → hex id, value → {"v": hex}, caused_by = prev node hash.
#[no_mangle]
pub extern "C" fn nedb_put(
    ...
    #[cfg(feature = "phase2")]
    {
        ...
        // Causal chain: caused_by = previous version's content hash
...
/// Delete `key`.  Returns 0 on success (including key-not-found), -1 on error.
#[no_mangle]
pub extern "C" fn nedb_del(
    ...
    #[cfg(feature = "phase2")]
    {
        ...
        match h.db.delete(&h.coll, &key_id) {
            Ok(_)  => 0,
            Err(_) => -1,
        }
    }
...
/// Returns 1 if the database is empty, 0 if not, -1 on error.
#[no_mangle]
pub extern "C" fn nedb_is_empty(handle: *mut NedbHandle) -> c_int {
    ...
    {
        let h = unsafe { &*handle };
        if h.db.list(&h.coll).is_empty() { 1 } else { 0 }
    }
}
...
pub extern "C" fn nedb_batch_write(
    ...
    #[cfg(feature = "phase2")]
    {
        ...
        for op in ops_slice {
            ...
            let key_id  = hex::encode(k);

            if op.value.is_null() {
                let _ = h.db.delete(&h.coll, &key_id);
            } else {
                ...
                let caused_by = h.db.get(&h.coll, &key_id)
                    ...
                    .unwrap_or_default();
                let data = json!({ "v": val_hex });
                let _ = h.db.put(&h.coll, &key_id, data, caused_by, None, None);
            }
        }
        // Flush MANIFEST after batch so state root reflects all writes
        #[cfg(feature = "phase2")]
...
/// Returns the current BLAKE2b state root as a null-terminated hex string.
///
/// Phase 1: BLAKE2b-512 chain (128 hex chars) — advances on every write.
/// Phase 2: NEDB MANIFEST Merkle root (64 hex chars) — deterministic
///          commitment to all collection HEADs.  Identical across nodes
///          that have processed the same ITC chain.
///
/// Caller must free via `nedb_free_str`.
#[no_mangle]
...
/// Create a snapshot iterator over the current store.
...
pub extern "C" fn nedb_iter_new(handle: *mut NedbHandle) -> *mut NedbIter {
    ...
    {
        let h = unsafe { &*handle };
        // Decode hex ids back to binary keys; sort by binary key order
...
/// Seek to the first key >= `key`.  Returns 1 if valid, 0 if exhausted.
#[no_mangle]
...
/// Returns 1 if the iterator is at a valid entry.
#[no_mangle]
...
/// Copy the current key.  Returns 0 on success; caller frees via nedb_free_value.
#[no_mangle]
...
/// Copy the current value.  Returns 0 on success; caller frees via nedb_free_value.
#[no_mangle]
...
// ─────────────────────────────────────────────────────────────────────────
// Tests — validate the public C API in both phases
// ─────────────────────────────────────────────────────────────────────────
...
mod tests {
    ...
    fn test_iterator_ordered() {
        ...
        assert_eq!(keys, vec![b'a', b'b', b'c'], "must iterate in ascending binary order");
        nedb_iter_free(iter);
        nedb_close(h);
    }

    /// THE CONSENSUS PROPERTY: two nodes processing identical block sequences
    /// must arrive at identical state roots.  This is the storage-layer
    /// consensus proof that replaces the network-only Bitcoin model.
    #[test]
    fn test_head_determinism_is_the_consensus_property() {
        ...
        assert_eq!(head1, head2,
            "CONSENSUS FAILURE: identical writes produced different state roots");
        nedb_close(h1);
        nedb_close(h2);
    }
}
